...
- Do not hardcode stale frontier-model overrides for Codex native child agents. If an explicit frontier override is necessary, use the current frontier default from `OMX_DEFAULT_FRONTIER_MODEL` / the repo model contract, not older values such as `gpt-5.2`.
...
| --- | --- | --- | --- |
| Frontier (leader) | repo/user default | high | Primary leader/orchestrator for planning, coordination, and frontier-class reasoning. |
| Spark (explorer/fast) | repo/user default spark | low | Fast triage, explore, lightweight synthesis, and low-latency routing. |
| Standard (subagent default) | repo/user default | high | Default standard-capability model for installable specialists and secondary worker lanes unless a role is explicitly frontier or spark. |
| `explore` | repo/user default spark | low | Fast codebase search and file/symbol mapping (fast-lane, fast) |
| `analyst` | repo/user default | medium | Requirements clarity, acceptance criteria, hidden constraints (frontier-orchestrator, frontier) |
| `planner` | repo/user default | medium | Task sequencing, execution plans, risk flags (frontier-orchestrator, frontier) |
| `architect` | repo/user default | high | System design, boundaries, interfaces, long-horizon tradeoffs (frontier-orchestrator, frontier) |
| `debugger` | repo/user default | high | Root-cause analysis, regression isolation, failure diagnosis (deep-worker, standard) |
| `executor` | repo/user default | medium | Code implementation, refactoring, feature work (deep-worker, standard) |
| `team-executor` | repo/user default | medium | Supervised team execution for conservative delivery lanes (deep-worker, frontier) |
| `verifier` | repo/user default | high | Completion evidence, claim validation, test adequacy (frontier-orchestrator, standard) |
| `style-reviewer` | repo/user default spark | low | Formatting, naming, idioms, lint conventions (fast-lane, fast) |
| `quality-reviewer` | repo/user default | medium | Logic defects, maintainability, anti-patterns (frontier-orchestrator, standard) |
| `api-reviewer` | repo/user default | medium | API contracts, versioning, backward compatibility (frontier-orchestrator, standard) |
| `security-reviewer` | repo/user default | medium | Vulnerabilities, trust boundaries, authn/authz (frontier-orchestrator, frontier) |
| `performance-reviewer` | repo/user default | medium | Hotspots, complexity, memory/latency optimization (frontier-orchestrator, standard) |
| `code-reviewer` | repo/user default | high | Comprehensive review across all concerns (frontier-orchestrator, frontier) |
| `dependency-expert` | repo/user default | high | External SDK/API/package evaluation (frontier-orchestrator, standard) |
| `test-engineer` | repo/user default | medium | Test strategy, coverage, flaky-test hardening (deep-worker, frontier) |
| `quality-strategist` | repo/user default | medium | Quality strategy, release readiness, risk assessment (frontier-orchestrator, standard) |
| `build-fixer` | repo/user default | high | Build/toolchain/type failures resolution (deep-worker, standard) |
| `designer` | repo/user default | high | UX/UI architecture, interaction design (deep-worker, standard) |
| `writer` | repo/user default | high | Documentation, migration notes, user guidance (fast-lane, standard) |
| `qa-tester` | repo/user default | low | Interactive CLI/service runtime validation (deep-worker, standard) |
| `git-master` | repo/user default | high | Commit strategy, history hygiene, rebasing (deep-worker, standard) |
| `code-simplifier` | repo/user default | high | Simplifies recently modified code for clarity and consistency without changing behavior (deep-worker, frontier) |
| `researcher` | repo/user default | high | External documentation and reference research (fast-lane, standard) |
| `product-manager` | repo/user default | medium | Problem framing, personas/JTBD, PRDs (frontier-orchestrator, standard) |
| `ux-researcher` | repo/user default | medium | Heuristic audits, usability, accessibility (frontier-orchestrator, standard) |
| `information-architect` | repo/user default | low | Taxonomy, navigation, findability (frontier-orchestrator, standard) |
| `product-analyst` | repo/user default | low | Product metrics, funnel analysis, experiments (frontier-orchestrator, standard) |
| `critic` | repo/user default | high | Plan/design critical challenge and review (frontier-orchestrator, frontier) |
| `vision` | repo/user default | low | Image/screenshot/diagram analysis (fast-lane, frontier) |
<!-- OMX:MODELS:END -->
